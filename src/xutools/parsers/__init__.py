...
class XPathParser():
    ...
    stepExpr << Group(Literal('.') | \
        abbrevForwardStep).setResultsName("cf_match") + \
...
class TEILiteParser():
    ...
    sssRegion = sssStart + Group(Group(headRegion) + \
        ZeroOrMore(Group(pRegion) | Group(commentRegion) | \
            Group(tableRegion))) + MatchFirst(sssEnd)
    ...
    ssRegion = ssStart + Group(Group(headRegion) + ZeroOrMore(Group(pRegion) | \
        Group(commentRegion) | Group(sssRegion) | Group(tableRegion))) + \
            MatchFirst(ssEnd)
    ...
    sRegion = sStart + Group(Group(headRegion) + ZeroOrMore(Group(pRegion) | \
        Group(commentRegion) | Group(ssRegion) | Group(tableRegion))) + \
            MatchFirst(sEnd)
    ...
    def getRegionID(self, subtree_parse, region_type, subtree_idx):
        result = None
        if ("subsubsection" == region_type or "subsection" == region_type or \
            "section" == region_type):
...
class CiscoIOSParser():
    ...
    interfaceStart = LineStart() + Group(Literal("interface") + restOfLine) + \
        LineEnd()
    ...
    def getRegionID(self, subtree_parse, region_type, subtree_idx):
        ...
        if 'interface' == region_type:
            ...
            result = "root"
        elif 'router' == region_type:
            if subtree_parse[0][0] == 'router':
                result = subtree_parse[0][1].strip()
            else:
                print 'Unrecognized parse for router ' + str(subtree_parse)
        elif 'address-family' == region_type:
            if subtree_parse[0][0] == 'address-family':
                result = subtree_parse[0][1].strip()
            elif subtree_parse[0][1] == 'address-family':
                result = subtree_parse[0][2].strip()
            else:
                print 'Unrecognized parse for address-family' + str(subtree_parse)
        else:
            print "unrecognized region type: " + region_type
            sys.exit(-1)
    ...
    def getGrammarForUnit(self, report_unit):
        ...
        elif 'router' == report_unit:
            regionGrammar = self.router_region
        elif 'address-family' == report_unit:
            regionGrammar = self.address_family_region
        else:
            print("Unknown Grammar error!\n")
            sys.exit(-1)
...
## The JuniperParser class is used to parse Juniper config files encoded
#   in TEI-XML.
#
#  @note The grammar reference is 'juniper'
class JuniperParser():
    LCURL = Suppress('{')
    RCURL = Suppress('}')
    SEMI = Suppress(';')
    ...
    kw = Group(OneOrMore(word)) + FollowedBy('{')
    kw.ignore(COMMENT)

    block = Forward()
    block << Group(kw + LCURL + Group(ZeroOrMore(block | stmt)) + RCURL)

    groups = Keyword('groups') + FollowedBy('{') + LCURL + \
        Group(ZeroOrMore(block | stmt)) + RCURL
    interfaces = Keyword('interfaces') + FollowedBy('{') + LCURL + \
        Group(ZeroOrMore(block | stmt)) + RCURL
    apply_groups = Keyword('apply-groups') + RBRACK + Group(ZeroOrMore(word)) +\
        LBRACK + SEMI
...
class SubsetCParser():
    ...
    fundecl = Group(TYPE + NAME + LPAR + Optional(Group(delimitedList(arg))) +
        RPAR + LBRACE + Group(body) + RBRACE)
